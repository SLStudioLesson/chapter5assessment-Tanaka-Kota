...
public class TaskUI {
    ...
    public void inputNewInformation() {
        ...
        while (flg) {
            try {
                ...
                if (!(name.length() <= 10)) {
                    System.err.println("タスク名は10文字以内で入力してください");
                    System.out.println();
                    continue;
                }
                ...
                taskLogic.save(Integer.parseInt(code),name,Integer.parseInt(code2),loginUser);
                flg = false;
            } catch (IOException e) {
                e.printStackTrace();
            } catch (AppException e) {
    ...
    public void inputChangeInformation() {
        ...
        while(flg) {
            try {
                ...
                if(!(Integer.parseInt(code2) == 1 || Integer.parseInt(code2) == 2)) {
                    System.out.println("ステータスは1・2の中から選択してください");
                    System.out.println();
                    continue;
                }
                taskLogic.changeStatus(Integer.parseInt(code),Integer.parseInt(code2),loginUser);
            } catch (IOException e ) {
                e.printStackTrace();
            } catch (AppException e) {
                System.out.println(e.getMessage());
    ...
    //     while (flg) {
    //         try {
    //             System.out.print("削除する登録コードを入力してください: ");
    //             String code = reader.readLine();
    //             if (!isNumeric(code)) {
